package com.mentorship.mentordashboard.controller;
...
public class MentorDashboardController {
    ...
    public ResponseEntity<ApiResponseDTO<MentorProfileDTO>> getMentorProfile(@PathVariable Long mentorId) {
        logger.info("GET /api/mentor/dashboard/{}/profile - Fetching mentor profile", mentorId);
        try {
            MentorProfileDTO profile = dashboardService.getMentorProfile(mentorId);
            logger.info("Mentor profile fetched successfully for mentorId: {}", mentorId);
            return ResponseEntity.ok(ApiResponseDTO.success(profile));
        } catch (Exception e) {
            logger.error("Error fetching mentor profile for mentorId: {} - {}", mentorId, e.getMessage());
            return ResponseEntity.badRequest().body(ApiResponseDTO.error(e.getMessage()));
        }
    }
