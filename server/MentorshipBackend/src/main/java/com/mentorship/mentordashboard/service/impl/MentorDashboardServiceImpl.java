...
import com.mentorship.mentordashboard.repository.FeedbackRepository;
...
import com.mentorship.mentordashboard.repository.SessionRepository;
import com.mentorship.mentordashboard.repository.TransactionRepository;
import com.mentorship.mentordashboard.service.MentorDashboardService;
...
public class MentorDashboardServiceImpl implements MentorDashboardService {
    ...
    public MentorProfileDTO getMentorProfile(Long mentorId) {
        ...
        Double averageRating = feedbackRepository.calculateAverageRating(mentorId);
        Integer totalStudents = mentorStudentRepository.countActiveStudents(mentorId);
        Integer totalSessions = sessionRepository.countTotalSessionsByMentor(mentorId);

        return MentorProfileDTO.builder()
                .mentorId(mentor.getMentorId())
                .userId(mentor.getUserDetails().getUserId())
                .firstName(mentor.getUserDetails().getFirstName())
                .lastName(mentor.getUserDetails().getLastName())
                .email(mentor.getUserDetails().getEmail())
                .phoneNo(mentor.getUserDetails().getPhoneNo())
                .specialization(mentor.getSpecialization())
                .experience(mentor.getExperience())
                .ratePerSession(mentor.getRatePerSession())
                .discountPercent(mentor.getDiscountPercent())
                .verificationStatus(mentor.getVerificationStatus() != null ? mentor.getVerificationStatus().name() : null)
                .averageRating(averageRating != null ? Math.round(averageRating * 10.0) / 10.0 : 0.0)
