...
import { getConversationWithMentor, markMessagesAsReadByStudent } from '../../services/studentService';
...
const StudentChatModal = ({ isOpen, onClose, studentId }) => {
  ...
  const loadMessages = async (mentorId, silent = false) => {
    try {
      if (!silent) {
        console.log('Loading messages for mentorId:', mentorId, 'studentId:', studentId);
      }
  ...
  const sendMessage = async () => {
    if (!newMessage.trim() || !selectedMentor || !selectedMentor.mentorId) return;

    try {
      console.log('Sending message:', { studentId, mentorId: selectedMentor.mentorId, content: newMessage });
                ...
                {mentors.map((mentor) => (
                  ...
                  >
                    ...
                    <div className="chat-conversation-info">
                      <div className="chat-conversation-name">{mentor.name}</div>
                      <div className="chat-conversation-preview">{mentor.lastMessage}</div>
                    </div>
                    {mentor.unread > 0 && (
            ...
            {selectedMentor ? (
              <>
                <div className="chat-messages-header">
                  <div className="chat-avatar">👨‍🏫</div>
                  <div>
                    <div className="chat-header-name">{selectedMentor.name}</div>
