...
const BrowseMentors = ({ onBack, onNavigateToSubscriptions }) => {
    ...
    const fetchMentors = async () => {
        try {
            ...
            if (err.response && err.response.data && err.response.data.message && err.response.data.message.includes("Active subscription required")) {
                console.log("DEBUG: Caught Subscription Error");
                setError("You need an active subscription to browse mentors. Please upgrade your plan.");
            } else {
    ...
    const domains = ["All", ...new Set(mentors.map((m) => m.specialization).filter(Boolean))];
    ...
    return (
        <div className="browse-mentors-container">
            ...
            <p className="subtitle">Find and connect with industry experts to accelerate your learning.</p>
                        ...
                        <div key={mentor.mentorId} className="mentor-card">
                            <div className="mentor-card-header">
                                ...
                                <div className="mentor-info">
                                    <h3>{mentor.name}</h3>
                                    <div className="mentor-domain">{mentor.specialization || "General"}</div>
                                    <div className="mentor-experience">{mentor.experience} Exp</div>
                                </div>
                            </div>
                            ...
                            <div className="mentor-card-body">
                                <div className="mentor-rating">
                                    ...
                                    <span style={{ fontWeight: "normal", color: "#888", fontSize: "12px", marginLeft: "4px" }}>
                                        (Rating)
                                    </span>
                                </div>
                                <div className="mentor-about">
                                    {mentor.about || "Experienced professional ready to mentor students."}
                                </div>
                            </div>
            ...
            {showProfileModal && selectedMentor && (
                <div className="mentor-modal-overlay" onClick={() => setShowProfileModal(false)}>
                    <div className="mentor-modal-content" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="modal-body-content">
                            <div className="modal-profile-info">
                                ...
                                <div className="modal-domain">{selectedMentor.specialization}</div>
                            </div>

                            <div className="modal-stats-row">
                                <div className="stat-item">
                                    <div className="stat-value">⭐ {selectedMentor.rating || "N/A"}</div>
                                    <div className="stat-label">Rating</div>
                                </div>
                                <div className="stat-item">
                                    <div className="stat-value">{selectedMentor.experience || "N/A"}</div>
                                    <div className="stat-label">Experience</div>
                                </div>
                                <div className="stat-item">
                                    <div className="stat-value">₹{selectedMentor.ratePerSession || 500}</div>
                            ...
                            <div className="modal-section">
                                <h4>📧 Contact</h4>
                                <p style={{ color: '#555' }}>{selectedMentor.email || "Email not available"}</p>
                            </div>
