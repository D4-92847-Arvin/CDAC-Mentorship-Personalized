...
function MCQPractice() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (activeTab === 'sessions') {
        ...
        const sessionQuestions = response.data?.filter(q => q.sessionId === currentSession.sessionId) || [];
  ...
  const handleCreateSession = async (e) => {
    ...
    if (sessions.length >= 10) {
      handleApiError(
        { response: { data: { message: 'Maximum limit of 10 practice sessions per student reached.' } } },
  ...
  const handleCreateQuestion = async (e) => {
    ...
    if (questions.length >= 10) {
      handleApiError(
        { response: { data: { message: 'Maximum limit of 10 MCQ questions per student reached.' } } },
  ...
  return (
    <div className="mcq-practice-page">
      <div className="page-header">
        <button className="back-button" onClick={() => navigate('/mentor/students')}>
          ← Back to Students
        </button>
        <h1 className="page-title">MCQ Practice</h1>
        <p className="page-subtitle">Manage practice questions and track progress</p>
      </div>
            ...
            <div className="sessions-section">
              <div className="section-header">
                ...
                <button 
                  ...
                  title={sessions.length >= 10 ? 'Maximum 10 sessions limit reached' : 'Create new session'}
                >
              ...
              <div className="sessions-grid">
                {sessions.length === 0 ? (
                  <div className="empty-state">No practice sessions created yet</div>
                ) : (
                  sessions.map((session) => (
                    ...
                    >
                      <div className="session-header">
                        ...
                        {session.isCompleted && <span className="badge-completed">✓ Completed</span>}
                        <button 
                          className="delete-btn"
                          onClick={(e) => {
                            e.stopPropagation();
                            if (window.confirm('Delete this session and all its questions?')) {
                      ...
                      <div className="session-stats">
                        ...
                        <span>🎯 {session.accuracyPercentage?.toFixed(0)}% Accuracy</span>
                      </div>
                    </div>
                  ))
                )}
            ...
            <div className="questions-section">
              <div className="section-header">
                ...
                <button 
                  ...
                  title={questions.length >= 10 ? 'Maximum 10 questions limit reached' : 'Create new question'}
                >
                  ...
                  questions.map((q, index) => (
                    <div key={q.questionId} className="question-card">
                      <div className="question-header">
                        ...
                        <span className={`difficulty ${q.difficultyLevel?.toLowerCase()}`}>
                          {q.difficultyLevel}
                        </span>
                      ...
                      <div className="options">
                        <div className={q.correctAnswer === 'A' ? 'option correct' : 'option'}>
                          A. {q.optionA}
                        </div>
                        <div className={q.correctAnswer === 'B' ? 'option correct' : 'option'}>
                          B. {q.optionB}
                        </div>
                        <div className={q.correctAnswer === 'C' ? 'option correct' : 'option'}>
                          C. {q.optionC}
                        </div>
                        <div className={q.correctAnswer === 'D' ? 'option correct' : 'option'}>
                          D. {q.optionD}
                        </div>
                      </div>
                      {q.explanation && (
